...
export default function CalendarPage() {
  ...
  const headerLabel = () => {
    if (view === "month")
      return `${MONTHS_SHORT[cursor.getMonth()]} ${cursor.getFullYear()}`;
    if (view === "week") {
      ...
      >
        ...
        >
          ...
          <button
            ...
            style={{ ...S.btnGhost, padding: "2px 8px", borderRadius: 8 }}
          >
            ✕
          </button>
        </div>
        ...
        >
          <button
            onClick={() => setJumpCursor(new Date(jYear, jMonth - 1, 1))}
            style={S.btnGhost}
          >
            ‹
          </button>
        ...
        >
          {cells.map((d, i) => {
            ...
            const isT =
              d === today.getDate() &&
              jMonth === today.getMonth() &&
              jYear === today.getFullYear();
            const isSel =
              d === cursor.getDate() &&
              jMonth === cursor.getMonth() &&
              jYear === cursor.getFullYear();
            return (
              <button
                key={d}
                onClick={() => {
                  setCursor(new Date(jYear, jMonth, d));
                  setShowJump(false);
                }}
                style={{
                  textAlign: "center",
        ...
        <div style={{ flex: 1, overflow: "auto" }}>
          {HOURS.map((h) => (
            ...
            >
              ...
              {dates.map((date, di) => {
                const dayCards = cardsForDay(date).filter(
                  (c) => new Date(c.dueDate).getHours() === h,
                );
                return (
                  <div
                    key={di}
                  ...
                  >
                    {dayCards.map((c, ci) => (
                      <span
                        ...
                        title={c.title}
                        style={{
                          ...S.eventChip(CHIP_COLORS[ci % CHIP_COLORS.length]),
                          fontSize: 11,
                        }}
                      >
                        {c.title}
  ...
  const DayView = () => (
    <div style={{ flex: 1, overflow: "auto" }}>
      ...
      >
        {DAYS_SHORT[cursor.getDay()]}, {MONTHS[cursor.getMonth()]}{" "}
        {cursor.getDate()}, {cursor.getFullYear()}
      </div>
      {HOURS.map((h) => {
        const dayCards = cardsForDay(cursor).filter(
          (c) => new Date(c.dueDate).getHours() === h,
        );
        return (
          <div
          ...
          >
            ...
            <div
              style={{ borderLeft: "1px solid #2d2d4e", padding: "4px 8px" }}
            >
              {dayCards.map((c, ci) => (
                <span
                  key={c._id}
  ...
  return (
    <div style={S.page}>
              ...
              >
                ...
                >
                  ...
                  <button
                    ...
                    style={{
                      padding: "8px",
                      background: "linear-gradient(135deg,#EB5A46,#c94234)",
                      color: "#fff",
                      borderRadius: 6,
                      fontWeight: 700,
                      fontSize: 13,
      ...
      <div style={S.controls}>
        ...
        <button onClick={goBack} style={{ ...S.btnGhost, padding: "5px 10px" }}>
          ‹
        </button>
        <button onClick={goToday} style={S.btn}>
          Today
        </button>
        <button onClick={goNext} style={{ ...S.btnGhost, padding: "5px 10px" }}>
          ›
        </button>
                ...
                {["day", "week", "month"].map((v) => (
                  <button
                    ...
                    style={{
                      ...
                      textAlign: "left",
                      background:
                        view === v ? "rgba(79,142,247,.15)" : "transparent",
                      color: "#e0e0e0",
                      border: "none",
                      cursor: "pointer",
                      fontSize: 14,
